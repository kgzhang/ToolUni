...
class TargetValidationOrchestrator:
    ...
    def run(self, target: str, disease: str = None, modality: str = None) -> Dict:
        ...
        # Create timestamped output directory in PROJECT ROOT (not skill folder)
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_dir = _PROJECT_ROOT / 'reports' / f'run_{target}_{timestamp}'
    ...
    def _generate_visualizations(self, results: Dict) -> List[str]:
        ...
        if gtex_data:
            tissues = []
            for e in gtex_data:
                tissue = e.get('tissue', 'Unknown')
                # Handle both string and dict formats
                if isinstance(tissue, dict):
                    tissue_name = tissue.get('name', 'Unknown')
                else:
                    tissue_name = tissue
                tissues.append({'tissue': tissue_name, 'tpm': e.get('tpm', 0)})
