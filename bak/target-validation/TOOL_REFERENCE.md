...
## CRITICAL: ToolUniverse SDK Usage

**Implementation**: See [scripts/tool_utils.py](scripts/tool_utils.py) for utility functions.

**API Pattern**:
1. Initialize ToolUniverse and call `load_tools()`
2. Access tools via `tu.tools.ToolName(parameter="value")`
3. Handle response formats using `ToolResponseHandler.handle_response()`

**No run_batch method** - execute calls sequentially or use threading manually.

---

## CRITICAL Parameter Corrections

These corrections are ESSENTIAL for getting data. Using wrong parameters will return empty results or validation errors.

### Open Targets Tools (Most Common Errors)

| Tool | Correct Parameter | WRONG Parameter | Impact |
|------|-------------------|-----------------|--------|
| `OpenTargets_get_diseases_phenotypes_by_target_ensembl` | `ensemblId` | `ensembl` | Returns validation error |
| `OpenTargets_get_target_tractability_by_ensemblID` | `ensemblId` | `ensemblID` | Returns validation error |
| `OpenTargets_get_target_safety_profile_by_ensemblID` | `ensemblId` | `ensemblID` | Returns validation error |
| `OpenTargets_get_target_interactions_by_ensemblID` | `ensemblId` | `ensemblID` | Returns validation error |
| `OpenTargets_get_target_gene_ontology_by_ensemblID` | `ensemblId` | `ensemblID` | Returns validation error |
| `OpenTargets_get_publications_by_target_ensemblID` | `entityId` | `ensemblId` | CRITICAL! Different param name |
| `OpenTargets_get_biological_mouse_models_by_ensemblID` | `ensemblId` | `ensemblID` | Returns validation error |
| `OpenTargets_get_chemical_probes_by_target_ensemblID` | `ensemblId` | `ensemblID` | Returns validation error |
| `OpenTargets_get_associated_drugs_by_target_ensemblID` | `ensemblId` + `size` | `ensemblId` only | Returns limited data |
| `OpenTargets_get_target_classes_by_ensemblID` | `ensemblId` | `ensemblID` | Returns validation error |
| `OpenTargets_get_target_homologues_by_ensemblID` | `ensemblId` | `ensemblID` | Returns validation error |
| `OpenTargets_get_target_constraint_info_by_ensemblID` | `ensemblId` | `ensemblID` | Returns validation error |
| `OpenTargets_multi_entity_search_by_query_string` | `queryString` | `query` | Returns validation error |

### Other Common Parameter Corrections
...
**Implementation**: See `ToolResponseHandler` in [scripts/tool_utils.py](scripts/tool_utils.py).
...
|------|---------|---------------------|-------|
| `OpenTargets_get_diseases_phenotypes_by_target_ensembl` | Disease associations | `ensemblId` | Standard |
| `OpenTargets_get_target_tractability_by_ensemblID` | Tractability | `ensemblId` (NOT `ensemblID`) | Case-sensitive! |
| `OpenTargets_get_target_safety_profile_by_ensemblID` | Safety liabilities | `ensemblId` (NOT `ensemblID`) | Case-sensitive! |
| `OpenTargets_get_target_interactions_by_ensemblID` | PPI network | `ensemblId` (NOT `ensemblID`) | Case-sensitive! |
| `OpenTargets_get_target_gene_ontology_by_ensemblID` | GO annotations | `ensemblId` (NOT `ensemblID`) | Case-sensitive! |
| `OpenTargets_get_publications_by_target_ensemblID` | Literature | **`entityId`** (NOT `ensemblId`) | CRITICAL! Different param |
| `OpenTargets_get_biological_mouse_models_by_ensemblID` | KO phenotypes | `ensemblId` (NOT `ensemblID`) | Case-sensitive! |
| `OpenTargets_get_chemical_probes_by_target_ensemblID` | Chemical probes | `ensemblId` (NOT `ensemblID`) | Case-sensitive! |
| `OpenTargets_get_associated_drugs_by_target_ensemblID` | Known drugs | `ensemblId`, **`size`** | size REQUIRED |
| `OpenTargets_get_target_classes_by_ensemblID` | Target class | `ensemblId` (NOT `ensemblID`) | Case-sensitive! |
| `OpenTargets_get_target_homologues_by_ensemblID` | Paralogs | `ensemblId` (NOT `ensemblID`) | Case-sensitive! |
| `OpenTargets_get_target_constraint_info_by_ensemblID` | Constraint info | `ensemblId` (NOT `ensemblID`) | Case-sensitive! |
| `OpenTargets_target_disease_evidence` | Detailed evidence | **`ensemblId` + `efoId`** | BOTH required |
| `OpenTargets_multi_entity_search_by_query_string` | Search entities | **`queryString`** (NOT `query`) | CRITICAL! |
...
|------------------|-------------|-------|
| `OpenTargets_get_diseases_phenotypes_by_target_ensembl` | `search_clinical_trials` + `Pharos_get_target` | Use clinical trials and Pharos for disease associations |
...
**Implementation**: See `get_versioned_ensembl_id()` in [scripts/tool_utils.py](scripts/tool_utils.py).

**Strategy**:
1. Get versioned ID from Ensembl lookup
2. Try versioned ID first with GTEx
3. Fall back to unversioned ID if versioned fails
...
**Implementation**: See `prepare_soap_tool_call()` in [scripts/tool_utils.py](scripts/tool_utils.py).
...
**Implementation**: See `ToolFallbackChain` class in [scripts/tool_utils.py](scripts/tool_utils.py).

**Pattern**:
1. Try primary tool with correct parameters
2. On failure, try fallback tools in sequence
3. Document all failures for reporting
4. Use `ToolCallResult` to track success/failure

**Standard Fallback Chains** (see `FALLBACK_CHAINS` in tool_utils.py):
- **Expression**: GTEx → HPA
- **PPI**: STRING → IntAct → BioGRID
- **Chemical Matter**: ChEMBL → BindingDB → DGIdb
- **Constraint**: gnomAD → OpenTargets
- **Literature**: PubMed → EuropePMC → PubTator3
- **Pathways**: Reactome → GO → WikiPathways