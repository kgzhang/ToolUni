...
class ValidationVisualizer:
    ...
    def plot_safety_dashboard(self, safety_data: Dict, save: bool = True) -> str:
        """
        Generate safety profile RADAR chart.
        """
        ...
        # 1. Critical Tissue Expression Bar Chart (top left)
        ax1 = fig.add_subplot(gs[0, 0])
        critical_tissues = ['Heart', 'Liver', 'Kidney', 'Brain', 'Bone Marrow']
        expr_levels = [safety_data.get('expression', {}).get(t.lower(), 0)
        ...
        ax1.barh(critical_tissues, expr_levels, color=colors, edgecolor='white')
        ax1.set_xlabel('TPM')
        ax1.set_title(self._t('critical_tissue'), fontweight='bold', fontsize=11)
        ax1.axvline(x=10, color=self.COLORS['danger'], linestyle='--', alpha=0.5)
        ax1.spines['top'].set_visible(False)
        ax1.spines['right'].set_visible(False)

        # 2. Genetic Validation Radar (top center)
        ax2 = fig.add_subplot(gs[0, 1], projection='polar')

        mouse_ko = safety_data.get('mouse_ko', 'unknown')
        pli = safety_data.get('gnomad_pli', 0)
        safety_score = safety_data.get('score', 0)

        # Radar chart for safety metrics
        categories = ['Mouse KO', 'pLI', 'Safety Score']
        values = [
            100 if mouse_ko == 'viable' else 0 if mouse_ko == 'lethal' else 50,
            (pli * 100) if pli else 50,
            (safety_score / 20) * 100
        ]
        ...
        ax2.fill(angles, values, color=self.COLORS['primary'], alpha=0.25)
        ax2.set_xticks(angles[:-1])
        ax2.set_xticklabels(categories, fontsize=9, fontweight='bold')
        ax2.set_ylim(0, 100)
        ax2.set_title(self._t('genetic_validation'), fontweight='bold', fontsize=11, pad=10)

        # 3. Safety Flags (top right)
        ax3 = fig.add_subplot(gs[0, 2])
        flags = safety_data.get('flags', [])
        if flags:
            for i, flag in enumerate(flags[:5]):
                ax3.text(0.1, 0.9 - i*0.2, f" {flag}", fontsize=10,
                        color=self.COLORS['danger'], fontweight='bold')
        else:
            ax3.text(0.5, 0.5, self._t('no_safety_flags'), ha='center', va='center',
                    fontsize=12, color=self.COLORS['success'], fontweight='bold')
        ax3.set_xlim(0, 1)
        ax3.set_ylim(0, 1)
        ax3.axis('off')
        ax3.set_title(self._t('safety_flags'), fontweight='bold', fontsize=11)

        # 4. Safety Score Gauge (bottom spanning)
        ax4 = fig.add_subplot(gs[1, :])

        max_score = 20
        percentage = safety_score / max_score * 100

        # Draw horizontal gauge
        ax4.barh([0], [safety_score], color=self.COLORS['success'] if percentage >= 70 else
                 self.COLORS['warning'] if percentage >= 40 else self.COLORS['danger'],
                 height=0.5, edgecolor='white', linewidth=2)
        ax4.barh([0], [max_score], color='#E9ECEF', height=0.5, alpha=0.3)
        ax4.set_xlim(0, max_score + 2)
        ax4.set_ylim(-0.5, 0.5)

        # Add score text
        ax4.text(safety_score + 0.5, 0, f'{safety_score}/{max_score}', va='center',
                fontsize=14, fontweight='bold')
        ax4.text(max_score/2, -0.35, f"{self._t('overall_safety')}: {percentage:.0f}%",
                ha='center', fontsize=12, fontweight='bold')

        ax4.axis('off')
        ax4.set_title(self._t('overall_safety'), fontweight='bold', fontsize=12)

        plt.suptitle(self._t('safety_dashboard'), fontsize=14, fontweight='bold', y=1.02)
        plt.tight_layout()
